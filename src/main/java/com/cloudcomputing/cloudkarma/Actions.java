...
public class Actions {
	...
	public static StartInstancesResult startInstances(List<String> instanceList) {
		AmazonEC2Client ec2Client = getEc2Client(Regions.US_WEST_2);
	...
	public static DescribeClustersResult describeClusters(List<String> clusterArnsList) {
		AmazonECSClient ecsClient = getEcsClient(Regions.US_WEST_2);
	...
	public static DescribeContainerInstancesResult describeContainerInstances(List<String> containerInstances) {
		...
		DescribeContainerInstancesResult result = ecsClient
	...
	public static boolean stopInstancesIfInactive(String cluster) {
		if (isZeroRunningTasksInCluster(cluster)) {
			...
			describeContainerInstances(containerInstancesList).getContainerInstances().forEach(containerInstance -> {
	...
	private static List<String> getAutoScalingGroupNameFromInstanceId(List<String> instanceIds) {
		...
		AmazonAutoScalingClient autoScalingClient = getAutoScalingClient(Regions.US_WEST_2);
		...
		result.getAutoScalingInstances().forEach(
				autoScalingInstance -> autoScalingGroupNameSet.add(autoScalingInstance.getAutoScalingGroupName()));
